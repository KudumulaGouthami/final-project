...
# ---------------- QUIZ ----------------
    ...
    if "quiz" not in st.session_state:
        ...
        num_q = st.number_input("Number of questions", 1, len(quizzes[cat]), value=min(5, len(quizzes[cat])))
    ...
    # Timer display + auto next
    total_time = 15
    elapsed = int(time.time() - st.session_state.start_time)
    remaining = max(0, total_time - elapsed)
    st.markdown(f"<div class='timer'>⏳ Time Left: {remaining} sec</div>", unsafe_allow_html=True)
    ...
    if page >= total:
        ...
        if st.button("Next ➡️"):
            ...
            feedback = ai_feedback(choice, q['answer'])
